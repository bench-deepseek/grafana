...
type Cfg struct {
	...
	// GnetProxyToken is the dedicated auth token for Grafana.com proxy requests and plugin installs.
	GnetProxyToken string
...
// ResolveGnetProxyToken must be called after OpenFeature is initialized.
// It sets GnetProxyToken to the dedicated token when the dedicatedGnetProxyToken
// flag is enabled and gnet_proxy_token is configured; otherwise falls back to sso_api_token.
func (cfg *Cfg) ResolveGnetProxyToken() {
	if openfeature.NewDefaultClient().Boolean(context.Background(), "dedicatedGnetProxyToken", false, openfeature.EvaluationContext{}) && cfg.GnetProxyToken != "" {
		return
	}
	cfg.GnetProxyToken = cfg.GrafanaComSSOAPIToken
}
...
func (cfg *Cfg) parseINIFile(iniFile *ini.File) error {
	...
	cfg.GnetProxyToken = valueAsString(iniFile.Section("grafana_com"), "gnet_proxy_token", "")
