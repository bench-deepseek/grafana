...
func Test_readGrafanaComSettings_GnetProxyToken(t *testing.T) {
	t.Run("reads gnet_proxy_token into GnetProxyToken when set", func(t *testing.T) {
		t.Setenv("GF_GRAFANA_COM_GNET_PROXY_TOKEN", "test-gnet-proxy-token")
		cfg := NewCfg()
		err := cfg.Load(CommandLineArgs{HomePath: "../../"})
		require.NoError(t, err)
		assert.Equal(t, "test-gnet-proxy-token", cfg.GnetProxyToken)
	})
}
