...
type Alertmanager interface {
	...
	IsExternalAMSyncActiveForOrg(ctx context.Context, orgID int64) (bool, error)
}
...
func (srv *ConvertPrometheusSrv) RouteConvertPrometheusPostAlertmanagerConfig(c *contextmodel.ReqContext, amCfg apimodels.AlertmanagerUserConfig) response.Response {
	...
	// tick.
	syncActive, err := srv.am.IsExternalAMSyncActiveForOrg(ctx, c.GetOrgID())
	if err != nil {
		logger.Error("Failed to check external AM sync status", "error", err)
		return response.Error(http.StatusInternalServerError, "failed to check external alertmanager sync status", err)
	}
	if syncActive {
